...
IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNELS = 84, 84, 4
...
def rgb2grayscale(image):
    """
    Convert the image from RGB to Grrayscale (This is what the NVIDIA model does)
    """
...
def batch_generator(data_dir, image_paths, actionss, batch_size, is_training):
    """
    ...
    while True:
        ...
        for index in np.random.permutation(image_paths.shape[0]):
            ...
            action = actionss[index]
            # argumentation
            if is_training and np.random.rand() < 0.6:
                image, steering_angle = augument(data_dir, center, left, right, action)
            else:
